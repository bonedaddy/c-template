...
/*! @typedef signature used by the thread_logger for log_fn calls
  ...
  * @param file_descriptor file descriptor to write log messages to, if 0 then only stdout is used
...
/*! @brief returns a new thread safe logger
  ...
  * @param with_debug whether to enable debug logging, if false debug log calls will be ignored
*/
thread_logger *new_thread_logger(bool with_debug);

/*! @brief returns a new file_logger
  * Calls new_thread_logger internally
  * @param output_file the file we will dump logs to. created if not exists and is appended to
*/
...
/*! @brief free resources for the file ogger
  * @param fhl the file_logger instance to free memory for. also frees memory for the embedded thread_logger and closes the open file
*/
...
/*! @brief logs a debug styled message - called by log_fn
  * @param thl pointer to an instance of thread_logger
  * @param file_descriptor file descriptor to write log messages to in addition to stdout logging. if 0 only stdout is used
